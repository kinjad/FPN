...
class AC_Network():
    def __init__(self,s_size,a_size,scope,trainer):
        with tf.variable_scope(scope):
            ...
            self.conv1 = slim.conv2d(activation_fn=tf.nn.elu, inputs=self.imageIn,num_outputs=32,                                     kernel_size=[5,5],stride=[1,1],padding='SAME')
            pool1 = slim.max_pool2d(self.conv1, [2, 2])
            self.conv2 = slim.conv2d(activation_fn=tf.nn.elu,
               inputs=self.conv1,num_outputs=32,
                                     kernel_size=[5,5],stride=[1,1],padding='SAME')
            pool2 = slim.max_pool2d(self.conv2, [2, 2])
            self.conv3 = slim.conv2d(activation_fn=tf.nn.elu,
               inputs=self.conv1,num_outputs=64,
                                     kernel_size=[4,4],stride=[1,1],padding='SAME')
            pool3 = slim.max_pool2d(self.conv3, [2, 2])
...
s_size = 7056  * 3# Observations are greyscale frames of 84 * 84 * 1
...
with tf.device("/cpu:0"): 
    ...
    for i in range(num_workers):
        workers.append(Worker('Pong-v0',i,s_size,a_size,trainer,model_path,global_episodes, retro_step))
