...
with tf.device("/gpu:0"):
    ...
    for data_index in range(data_size):
        ...
        data_frame_history.extend(one_past_history)
        data_frame_next.extend(next_observations)
        data_frame_reward.extend(rewards)
        data_frame_done.extend(dones)
    ...
    indices = [i for i in range(len(data_frame_history))]
    np.random.shuffle(indices)
    data_frame_history = [data_frame_history[i] for i in indices]
    data_frame_next = [data_frame_next[i] for i in indices]
    data_frame_reward = [data_frame_reward[i] for i in indices]
    data_frame_done = [data_frame_done[i] for i in indices]
    ...
    for epoch in range(num_epochs):
        ...
        for data_index in range(0, train_data_size, mini_batch_size):            
            ...
            feed_dict = {FPN.inputs:data_frame_history[sudo_data_index : sudo_data_index + mini_batch_size], FPN.true_observation:data_frame_next[sudo_data_index : sudo_data_index + mini_batch_size], FPN.true_reward:data_frame_reward[sudo_data_index : sudo_data_index + mini_batch_size], FPN.true_done:data_frame_done[sudo_data_index : sudo_data_index + mini_batch_size]}
        ...
        if epoch % 5 == 0 and epoch != 0:
            ...
            start_data_index = train_data_size
            end_data_index = train_data_size + test_data_size
            feed_dict = {FPN.inputs:data_frame_history[start_data_index : end_data_index], FPN.true_observation:data_frame_next[start_data_index : end_data_index], FPN.true_reward:data_frame_reward[start_data_index : end_data_index], FPN.true_done:data_frame_done[start_data_index : end_data_index]}
