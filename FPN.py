...
class FramePrediction_Network(object):
    def __init__(self, h_size, retro_step, trainer, scope):
        with tf.variable_scope(scope):
            ...
            self.conv1 = slim.conv2d(activation_fn=tf.nn.relu, inputs=self.imageIn, num_outputs=32, kernel_size=[6, 6], stride=[3, 3], padding='VALID')
            self.conv2 = slim.conv2d(activation_fn=tf.nn.relu, inputs=self.conv1, num_outputs=16, kernel_size=[4, 4], stride=[2, 2], padding='VALID')
